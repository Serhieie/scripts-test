const puppeteer = require('puppeteer');
const fs = require('fs');
const searchResult = require('../../../searchResultRobotaUa.json');
const sended = require('../../../appliedVacancies.json');
const letters = require('../../../letters/letters.json');
const { sleep } = require('../../../utils');
const { checkVacancyWorkUa } = require('../../../helpers');

//node ./scripts/rabotakUa/spam/spamJuniorFullStack.js

(async () => {
  const links = Object.values(searchResult);
  const applied = Object.values(sended);
  let linksArray = links.slice();
  let appliedVac = applied.slice();

  const browser = await puppeteer.launch({
    headless: false,
    defaultViewport: false,
    userDataDir: './tmp',
  });
  const page = await browser.newPage();

  for (let i = 0; i < linksArray.length; i++) {
    if (!appliedVac.includes(linksArray[i])) {
      const random =
        (Math.floor(Math.random() * 8) * (Math.random() * 2) + 1) * 1000;
      await page.goto(linksArray[i]);

      console.log(`1) Transition to ${linksArray[i]}`);
      await sleep(random);

      await page.waitForSelector(
        'div > div > div > div > div > div.pull-left > div > div > a',
        { visible: true }
      );

      const conditions = await checkVacancyWorkUa(page);

      if (conditions) {
        await page.click(
          'div > div > div > div > div > div.pull-left > div > div > a'
        );
        console.log('2) apply clicked! ');
        await sleep(random);

        await page.waitForSelector(
          '#savedresume > div:nth-child(2) > div:nth-child(3) > label',
          { visible: true }
        );
        await page.click(
          '#savedresume > div:nth-child(2) > div:nth-child(3) > label'
        );
        console.log('3) resume clicked');
        await page.click('#commonresume > div:nth-child(1) > div > label');
        console.log('4) letter clicked');

        const letter = letters.fullStackLetter;
        await page.evaluate(letter => {
          const element = document.querySelector('#addtext');
          if (element) {
            element.textContent = letter;
          } else {
            console.log('Element not found.');
          }
        }, letter);
        await sleep(random);
        await page.waitForSelector('#submitbtn', { visible: true });
        await page.click('#submitbtn');
        console.log('4) Send Resume ');
        await sleep(random);

        appliedVac.push(linksArray[i]);
        fs.appendFile(
          'appliedVacancies.json',
          JSON.stringify(linksArray[i]) + '\n',
          'utf8',
          err => {
            if (err) {
              console.error(
                'Помилка запису в файл appliedVacancies.json:',
                err
              );
              return;
            }
            console.log('Новий запис додано до файлу appliedVacancies.json.');
          }
        );
      }

      linksArray.shift();
      let result = JSON.stringify(linksArray);

      fs.writeFile('searchResultFullStack.json', result, function (err) {
        if (err) throw new Error(err);
      });
      console.log('4) json files were updated');
    }
  }

  await browser.close();
})();
